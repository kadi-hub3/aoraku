...
export const savedVideos = async(video) => {
  try {
    const {title} = video
    console.log(video,'video', typeof(title))
    const bookmarkVideo = await searchPosts(title)
    console.log(bookmarkVideo,'bookmarkedvideo')
    ...
    //   })
    //   console.log(savedVideo,'saved video')
    //   return savedVideo
  } catch (error) {
    throw new Error(error)
  }
